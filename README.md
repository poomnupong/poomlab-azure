...
| `min-consume` | `.github/workflows/min-consume.yml` | Weekly Sunday 00:00 GMT + manual | Deploys a minimal keep-alive footprint per subscription in West US 3 (`rg-min-consume-westus3`, VNET/subnet, NSG, Standard_B4as_v2 VM on latest Ubuntu LTS non-Pro ARM64 image). |
| `min-consume-teardown` | `.github/workflows/min-consume-teardown.yml` | Weekly Tuesday 00:00 GMT + manual | Deletes `rg-min-consume-westus3` in every targeted subscription (48 hours after `min-consume`). |
...
Use `min-consume` + `min-consume-teardown` for subscription activity keep-alive policy:

- Deploy: Sunday 00:00 GMT (`min-consume`)
- Teardown: Tuesday 00:00 GMT (`min-consume-teardown`, 48 hours later)
