...
| `CI_SP_OBJECT_ID`        | Object ID of the CI service principal, used by `landing-zone` to assign the Key Vault Secrets Officer role. Get: `az ad sp show --id "$AZURE_CLIENT_ID" --query id -o tsv` |
...
secrets that are injected into VMs at creation time.
